...
class BookingRepository(AbstractRepository[ViewingBooking]):
    ...
    async def get_for_tenant(self, tenant_id: int, status: Optional[str], page: int, page_size: int) -> Tuple[List[ViewingBooking], int]:
        ...
        result = await self.db.execute(
            ...
            .order_by(ViewingBooking.scheduled_at.desc())
            .offset((page - 1) * page_size).limit(page_size)
        )
        return list(result.scalars().all()), total

    async def get_for_landlord(self, landlord_id: int, status: Optional[str], page: int, page_size: int) -> Tuple[List[ViewingBooking], int]:
        ...
        result = await self.db.execute(q.order_by(ViewingBooking.scheduled_at.desc()).offset((page - 1) * page_size).limit(page_size))
        return list(result.scalars().all()), total
